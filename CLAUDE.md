...
- **Private dotfiles repository**: `opencode/sync-private.sh` clones or fast-forwards `git@github.com:irvj/dotfiles-private.git` to `~/.local/share/opencode/private/` using SSH. A not-yet-created repository is reported and skipped; other sync failures are fatal. `zshrc` loads `opencode.local.json` from that checkout through `OPENCODE_CONFIG`; override the repository or path with `DOTFILES_PRIVATE_REPO` or `DOTFILES_PRIVATE_DIR`.
